...
class DeepEnergyModel:
    ...
    reload_model [bool, str]: if True (bool), reloads the pretrained model with identical 
        hyperparameters and name (if exists in the CHECKPOINT folder). If a string, reloads  
        the model at the path indicated by the string.
    """
    def __init__(self,
                 cnn=None,
                 img_shape=(1, 28, 28), 
        ...
        # Model
        assert cnn is not None, "CNN model has to be specified!"
        self.img_shape = img_shape
        self.device = torch.device(device) if torch.cuda.is_available() else torch.device("cpu")
        print("Running on device:", self.device) 
        self.cnn = cnn(**CNN_args).to(self.device)
    ...
    def setup(self):
        ...
        # Hyperparams dict
        self.hparams_dict = {
            'cnn': self.cnn.__class__.__name__,
        ...
        if os.path.exists(self.ckpt_path) and self.reload_model is True:
            # Reload model with the same hyperparams and name
        ...
        elif isinstance(self.reload_model, str):
            # Reload model from a given path
            path = os.path.join(self.reload_model, "model_state_dict.pt")
            self.cnn.load_state_dict(torch.load(path))
            print("Loaded pretrained existsing model at %s" % self.reload_model)
